  ...
  router.get('/', isAuthenticated, isAdmin, async (req, res) => {
    ...
    await Course.find(criteria, function(err, doc){
        ...
        return res.status(200).json({
            ...
            data: {
                list: (resList.length < req.body.Page) ? resList[resList.length-1] : resList[req.body.Page],
                count: (resList.length < req.body.Page) ? resList[resList.length-1].length : resList[req.body.Page].length,
                page: (resList.length < req.body.Page) ? resList.length : req.body.page ,
...
router.post('/', isAuthenticated, isAdmin, async (req, res) => {
    ...
    await newCourse.save(function(err, result){
        if(err){
            return res.status(404).json({
                success: false,
                message: "The requested operation encountered an error!"
            })
        }
        return res.status(200).json({
            success: true,
...
router.put('/:id', isAuthenticated, isAdmin, async (req, res) => {
    await Course.findOneAndUpdate({_id: req.params['id']}, {title:req.body.title, unitCounts: req.body.unitsCount}, function(err, doc){
        if (err){
            return res.status(404).json({
                success: false,
...
router.post('/:id/Choose', isAuthenticated, isMaster, async (req, res) => {
    await Course.findById(req.params['id'], async function(err, doc){
        if (err){
            return res.status(404).json({
                success: false,
                message: "The requested course does not exist!"
